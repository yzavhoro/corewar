...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/25 16:07:32 by alazarev          #+#    #+#             */
/*   Updated: 2018/07/27 15:14:06 by alazarev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				f_lldi(t_map *map, t_carry *carry)
{
	...
	if (b1 * b2 && valid_treg(map->field[MV(carry->pos, b1 + b2 + 2)] - 1))
	{
		carry->reg[map->field[MV(carry->pos, b1 + b2 + 2)] - 1] =
			convert_to_reg(map->field, MV(carry->pos, sum));
		carry->carry = !map->field[MV(carry->pos, b1 + b2 + 2)];
	}
